...
def omnibus_score_test(
    ...
    grad = jax.grad(
        lambda params: nll(params, Y, N, Z_full, K, idx_full, include_const=False)
    )(params_full)
    hess = jax.hessian(
        lambda params: nll(params, Y, N, Z_full, K, idx_full, include_const=False)
    )(params_full)
    ...
    assert xi_idx is not None, "xi_idx should not be None for full model"
    ...
    # efficient score: subtract projection onto theta-gradient direction,
    # which is non-zero when the reduced fit hasn't fully converged
    # U = -(g_xi - I_Xt @ jnp.linalg.solve(I_tt_reg, g_theta))
    U = -g_xi
    ...
    Q = U.T @ J_inv @ U
    eig = eigvals
    pval = 1 - jax.scipy.stats.chi2.cdf(Q, df=len(xi_idx))
