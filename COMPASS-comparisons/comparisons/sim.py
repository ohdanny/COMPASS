import numpy as np
from config import S, K, ACTIVE_INTERACTIONS, SPATIAL_EXPR_VAR, SEED, XI_INT_SCALE
...
def guassian_kernel(coords: np.ndarray, lengthscale: float) -> np.ndarray:
...
def sample_N(
    ...
    eps = rng.normal(scale=spatial_eps, size=S)
    ...
    if not spatial:
        ...
        if gamma_scale is None:
            raise ValueError("Must provide gamma_scale when spatial=True")
            #random unit vector scaled by gamma_scale
        gamma = rand_unit(rng, R.shape[1]) * gamma_scale
        u_g = rng.multivariate_normal(mean=np.zeros(S), cov=K)
...
def simulate_counts(a_g,R,X,B,K,expr_setting,iso_setting,rep_id,rng ,params=None, spatial_expr_var=SPATIAL_EXPR_VAR):

    N = sample_N(a_g=a_g,R=R,K=K,rng=rng,spatial=True if expr_setting=="spatial" else False, spatial_eps=spatial_expr_var)
...
def load_true_params(T, r, K,xi_int_scale=XI_INT_SCALE):
    ...
    true_params = dict(
        ...
        xi_int=np.stack(
            [
                xi_int_scale * np.column_stack([rand_unit(g0, r) for _ in range(K - 1)])
                for _ in range(T-1)
            ],
            axis=1,
        ),  # shape (r, T-1, K-1)
