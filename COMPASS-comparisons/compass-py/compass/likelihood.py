...
def sample_Y(Pi:jnp.ndarray, kappa:jnp.ndarray, N:jnp.ndarray, rng: np.random.Generator) -> jnp.ndarray:
    """
    Generates Y ~ DM(N, Pi, kappa).
    Args:
        Pi: (S, K) matrix of isoform proportions for each spot. Rows should sum to 1.
        kappa: (S,) vector of concentration parameters for each spot. Higher kappa means less overdispersion.
        N: (S,) vector of total counts for each spot.
        rng: numpy random generator for reproducibility.
    Returns:    
        Y: (S, K) matrix of counts for each spot and isoform.
    """
    g = rng.gamma(jnp.maximum(kappa * Pi, 1e-8), 1.0)
...
def nll(
    ...
    eta = Z @ gammas
    eta = jnp.clip(eta, -40.0, 40.0)  # match R's eta clipping
    pi = softmax_ref(eta)
